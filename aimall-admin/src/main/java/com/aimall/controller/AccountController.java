    ...
    public ResponseVO login(
        ...
        try {
            if (!checkCode.equalsIgnoreCase((String) redisComponent.getCheckCode(checkCodeKey))) {
                throw new BusinessException("鍥剧墖楠岃瘉鐮佷笉姝ｇ‘");
            }
            if (StringTools.isEmpty(appConfig.getAdminAccount()) || StringTools.isEmpty(appConfig.getAdminPassword())) {
                throw new BusinessException("管理员账号未配置");
            }
            if (!account.equals(appConfig.getAdminAccount()) || !PasswordUtils.matches(password, appConfig.getAdminPassword())) {
